...
void main(void) {
  ...
  for (;;)
  {
    if (((PTJ == BUTTON_IS_PRESSED) || (PTJ == BUTTON2_IS_PRESSED)) && buttonLock == 0) //floor 3 button pressed
      {
        PORTA |= FLOOR_REQUESTED_LED_MASK;
        retCode = F3_Status(REQUEST_CAR);
        buttonLock = 1;
      }
    ...
    if (MSCAN_GotMsg()) //received message from EC
      ...
      if ((floorLocation & FLOOR_LOCATION_MASK) == FLOOR_3) //car is at floor three
        {
          FORCE_BITS(PORTA, LEDS_PORT_MASK, FLOOR3_LED_MASK); 
          PORTA &= ~FLOOR_REQUESTED_LED_MASK;
          buttonLock = 0;          
        }
        else if ((floorLocation & FLOOR_LOCATION_MASK) == FLOOR_2) //car is at floor one
        {
